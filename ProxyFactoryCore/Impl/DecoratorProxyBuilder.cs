...
namespace ProxyFactoryCore.Impl
{
    ...
    {
        private readonly IInterceptorConfiguration _interceptorConfiguration;
        ...
        public DecoratorProxyBuilder(IInterceptorConfiguration interceptorConfiguration = null)
        {
            _interceptorConfiguration = interceptorConfiguration;
        ...
        private MethodBuilder BuildMethod(MethodInfo prototype)
        {
            ...
            var interceptorTypes = _interceptorConfiguration.GetInterceptors(prototype);
            ...
            /* GET METHOD INFO AND STORE IT AT LOCATION 1 (Stloc_1) */
            ilGenerator.Emit(OpCodes.Ldtoken, prototype.DeclaringType);
            ilGenerator.Emit(OpCodes.Ldstr, prototype.Name);
            ilGenerator.Emit(OpCodes.Call, getMethod);
            ilGenerator.Emit(OpCodes.Stloc_1);
            /* END OF GET METHOD INFO AND STORE IT AT LOCATION 1 (Stloc_1) */

            for (var index = 0; index < interceptorTypes?.Length; index++)
            {
                var interceptorType = interceptorTypes[index];
                ...
                /* CREATE INTERCEPTOR INSTANCE AND STORE IT AT LOCATION 2 (Stloc_2) */
                ilGenerator.Emit(OpCodes.Newobj, interceptorCtor);
                ilGenerator.Emit(OpCodes.Stloc_2);
                /* END OF CREATE INTERCEPTOR INSTANCE AND STORE IT AT LOCATION 2 (Stloc_2) */
                ...
                /* END OF LOAD INPUT PARAMETERS INTO STACK */
            }
